...
function normalizeEmojis(text: string, emojis: string[]) {
  ...
  while ((m = EMOJI_MARKER_REGEX.exec(text)) !== null) {
    ...
    if (!Number.isNaN(idx) && emojis[idx] !== undefined && !used.includes(idx)) used.push(idx);
  }
...
// ===== Small UI pieces =====
const PreviewSection: React.FC<{ content: string; emojis: string[] }> = ({ content, emojis }) => {
  ...
  return (
    <div className="mb-3 p-3 rounded-xl bg-gradient-to-br from-default-50 to-default-100 border border-default-200 shadow-sm">
      ...
      <div className="bg-background/70 backdrop-blur-sm rounded-lg p-3 border border-default-200/50">
        <EmojiText text={content || ""} emojiUrls={emojis} className="text-default-700 leading-relaxed" />
        {content === "" && <div className="text-default-400 italic">Здесь появится ваш пост</div>}
      </div>
    </div>
  );
};

const EmojiList: React.FC<{ emojis: string[]; onRemove: (i: number) => void }> = ({ emojis, onRemove }) => {
      ...
      <div className="grid grid-cols-6 gap-3">
        {emojis.map((url, idx) => (
          <div key={`emoji-${idx}`} className="relative group">
            <img src={url} alt={`emoji-${idx}`} className="w-12 h-12 rounded-md object-cover border border-default-200" />
            <Button
              size="sm"
              color="danger"
              variant="flat"
            ...
            <div className="text-[10px] text-default-400 mt-1 text-center">[{`emoji:${idx}`}]</div>
          </div>
        ))}
      </div>
    </div>
...
const EditPostModal: React.FC<EditPostModalProps> = ({
  ...
  const removeEmojiAt = useCallback(
    (index: number) => {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      await updatePost({ id: postId, content: newText, emojiUrls: newEmojiUrls }).unwrap();
      if (onUpdated) await onUpdated();
      onClose();
    } catch (err: any) {
      setError(String(err?.data?.error || err?.message || "Не удалось обновить пост"));
    }
