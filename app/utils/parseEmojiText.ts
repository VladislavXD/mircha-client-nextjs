...
export const parseEmojiText = (
  ...
  while (i < text.length) {
    ...
    if (text[i] === "<") {
      ...
      if (closeTag !== -1) {
        ...
        if (tagContent === "<b>" || tagContent === "<strong>") {
          ...
          const actualClose =
            closeIndex !== -1 &&
            (closeIndex2 === -1 || closeIndex < closeIndex2)
              ? closeIndex
              : closeIndex2;
        ...
        if (
          ...
          if (actualClose !== undefined && actualClose !== -1) {
            ...
            if (text.slice(actualClose, actualClose + 9) === "</strike>")
              closeLength = 9;
            if (text.slice(actualClose, actualClose + 6) === "</del>")
              closeLength = 6;
    ...
    const nextTokenPos = (() => {
      ...
      const candidates = [a, b, c, d, e, f, g, h].filter(
        (x) => x !== -1 && x > i,
      );
    ...
    const chunk =
      nextTokenPos === -1 ? text.slice(i) : text.slice(i, nextTokenPos);
