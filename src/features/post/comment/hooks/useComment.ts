      ...
      if (context?.postId) {
        if (context.previousComments) {
          queryClient.setQueryData(
            ["comments", "post", context.postId],
            context.previousComments,
          );
        }
        if (context.previousPost) {
          queryClient.setQueryData(
            postKeys.detail(context.postId),
    ...
    onMutate: async (payload: string | { id: string; postId?: string }) => {
      ...
      const removeFromTree = (comments: any[], id: string): any[] =>
        comments
          ...
          .map((c: any) => ({
            ...c,
            replies: removeFromTree(c.replies || [], id),
          }));
...
export function useCreateReply() {
  ...
  return useMutation({
    ...
    onSettled: (
      data,
      error,
      payload: CreateCommentDto & { replyToId: string },
      context: any,
    ) => {
      // Инвалидируем query комментариев и поста
      if (context?.postId) {
