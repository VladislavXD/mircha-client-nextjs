 ...
 */
export const selectAllOnlineStatuses = (state: RootState) =>
  state.onlineStatus.statuses;
 ...
 */
export const selectMultipleOnlineStatuses =
...
export const selectOnlineUsersCount = (state: RootState): number => {
  return Object.values(state.onlineStatus.statuses).filter(
    (status) => status === true,
  ).length;
};
